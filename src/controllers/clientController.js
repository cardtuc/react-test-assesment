...
exports.editUser = (request, response) => {
  const { id, name, surname, age, dateOfBirth, phone, country } = request.body;
  const isEmptyParam = Client.checkParams(response, { name, surname, age, dateOfBirth, phone, id, country });
  if (isEmptyParam) {
    return
  }
  Client.readUsers(response).then(r => {
    ...
    if (index === -1) {
      ...
      r[index] = { ...r[index], name, surname, age, dateOfBirth, phone, country };
...
exports.addUser = (request, response) => {
  const { name, surname, age, dateOfBirth, phone, country } = request.body;

  const isEmptyParam = Client.checkParams(response, { name, surname, age, dateOfBirth, phone, country });
  if (isEmptyParam) {
    return
  }
  Client.readUsers(response).then(r => {
    ...
    if (isExists) {
      ...
      const newClient = new Client(name, surname, age, dateOfBirth, phone, country);
      newClient.addUser(r);
